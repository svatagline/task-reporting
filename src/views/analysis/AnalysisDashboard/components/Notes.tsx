...
import { mergeDetailForNotes, splitObjectValues } from "@/utils/helper";
...
export const Notes = ({ record = {}, allTaskData }: { record: any, allTaskData: INode[] }) => {
    ...
    useEffect(() => {
        ...
        try {
            const notes = allTaskData.find((i: any) => i?.name?.includes('note'))
            setAllNotes(notes ?? {})
            const otherDetailsList = mergeDetailForNotes(allTaskData.filter((i: any) => !Object.keys(i).includes('typing-speed')))
            console.log({ otherDetailsList })
            setOtherDetails(otherDetailsList)
        } catch (error) {
    ...
    const test = () => {
        ...
        const tempData = allTaskData.reduce((acc: any, record: INode) => {
            const mergedDetails = {
